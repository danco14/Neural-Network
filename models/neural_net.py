...
class NeuralNetwork:
    ...
    def backward(
        ...
        reg_loss = (reg / 2) * sq
        loss = (np.sum(-np.log(self.outputs["S"][np.arange(num_samples), y])) / num_samples) + reg_loss
        ...
        for n in range(self.num_layers, 0, -1):
            ...
            if n != 1:
                ...
                # Update weights
                self.params["W" + str(n)] = (1 - lr*reg)*self.params["W" + str(n)] - lr*(self.outputs["R" + str(n-1)].T @ upstream_grad)
        ...
        # Update weights
        self.params["W" + str(1)] = (1 - lr*reg)*self.params["W" + str(1)] - lr*(X.T @ upstream_grad)
        self.params["b" + str(1)] -= lr*(upstream_grad.T @ np.ones((num_samples,)))
